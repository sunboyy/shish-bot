...
with open('lib/word_to_index.pickle', 'rb') as handle:
    word_to_index = pickle.load(handle)

with open('lib/index_to_word.pickle', 'rb') as handle:
...
def initiate_model():
    ...
    # Load Model
    tl.files.load_and_assign_npz(sess=sess, name='lib/model.npz', network=net)
...
import pickle
with open('lib/lowerToCapital.pkl', 'rb') as handle:
